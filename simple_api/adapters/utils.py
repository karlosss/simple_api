...
def generate_w(adapter, extra_actions=None):
    extra_actions = extra_actions or {}
    ...
    adapter_instance = adapter(tuple(object_storage.storage.values()), extra_actions)
    api = adapter_instance.generate_api()
    weight_schema = adapter_instance.generate_weight_schema()
    return api, weight_schema
